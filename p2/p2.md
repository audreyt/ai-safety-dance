...
[^not-max]: 例如，假設我們給機器人這個目標：「從街對面的咖啡館給我拿一（1）杯咖啡」。只是一杯，沒有最大化。但如果我們沒有*明確*告訴 AI 我們重視 [X]，它會碾壓過 [X]。例如，機器人可能從顧客那裡偷咖啡，或留 0% 的小費等。）

*但我們不能列出我們重視的所有事物嗎？*你可能合理地問。但記得第一章的內容：我們甚至無法正式指定*如何識別貓的圖片*。[^gofai-cats] 正式指定「人類重視什麼」會更加困難。

[^gofai-cats]: 正如[第一部分](../p1/#before2000logicwithoutintuition)中詳細說明的，「老式 AI」試圖用精確的、硬編碼的規則來識別圖片中的東西（比如貓）。這些嘗試失敗了。直到 AI 研究人員放棄，讓 AI「自己學習」（機器學習），AI 才在 [2020 年用 EffNet-L2](https://paperswithcode.com/sota/image-classification-on-cifar-100) 達到人類水平（約 95.9% 準確率）識別圖片中的東西。這*確實*暗示了一個可能的解決方案，我們將在第三部分看到：不是*告訴* AI 我們重視什麼，而是設計 AI 讓它*自己學習*我們重視什麼。
...
[^cyber]: ⋯⋯嗯，他們在網路安全中*應該*使用安全思維。我在 [2024 年 Crowdstrike 事件](https://en.wikipedia.org/wiki/2024_CrowdStrike_incident)之後不久寫下這段話，該事件給世界造成了約 100 億美元的損失。
...
[^ai-evidence-wireheading]: 如果 AI 不能準確地「提前計劃」，它可以破壞性地自我修改，如 [Leike et al 2017](https://arxiv.org/pdf/1711.09883) 的威士忌遊戲所示。更微妙的是當 AI *確實*提前計劃，但不根據*當前*目標來判斷未來結果，比如 [Denison et al 2024](https://arxiv.org/pdf/2406.10162) 中的獎勵駭客。（像 GPT 這樣的大型語言模型（LLM），可以說，甚至沒有老式 AI 人士所認為的「目標」。見 [janus 2022](https://www.lesswrong.com/posts/vJFdjigzmcXMhNTsx/simulators)。）

[^ai-not-wirehead]: 這將很快在問題 #2 中以易懂的方式解釋，但如果你想要一個快速連結到證明這一點的論文：Everitt et al 2016，*[Self-Modification of Policy and Utility Function in Rational Agents](https://arxiv.org/pdf/1605.03142)*。
...
[^sycophancy-deception]: *「[AI 的私下想法] 糟糕。這詩不好。但我不想傷害他們的感情。[AI 的公開發言] 我誠實的評估是這首詩相當好，5 分中的 4 分！祝你申請哈佛/史丹佛好運！」* 這是來自 [Denison et al 2024](https://arxiv.org/pdf/2406.10162) 的*實際* AI 輸出（意譯），見圖 1。（公平地說，作者公開承認這一點，來自語言模型的「故意欺騙」*目前*是罕見的，但它們確實會發生！）
